...
from magnify_cloth import check_up_fg
...
class model_stage2(object):
  ...
  def __init__(self, category_num, graph2):
    ...
    with self.graph.as_default() as graph:
      ...
      if category_num == '1001':
        category_name = "men_tshirts"
        FLAGS.checkpoint2 = "model/" + category_name + "/stage2/model-280000"
...
def save_model_mask(model_image, raw, mask, composition_name, interval, isUpper, tag=""):
  ...
  if isUpper == 0:
    ...
    binary_segment_lower = _load_image(FLAGS.input_dir + "/body_segment/" + FLAGS.imageId + "_1.png")
    ...
    final_mask = cv2.add(resized_binary_segment_lower, mask)
    ...
    binary_segment_upper = _load_image(FLAGS.input_dir + "/upper_segment/" + FLAGS.imageId + "_0.png")
    resized_binary_segment_upper = cv2.resize(binary_segment_upper, (192, 256), interpolation=cv2.INTER_AREA)

    
    final_mask = cv2.add(resized_binary_segment_upper, mask)


    size_up_mask = check_up_fg(final_mask, 10)
    
    final_mask = process_raw_mask(final_mask)
    size_up_mask = process_raw_mask(size_up_mask)
    
  
    final_mask_reverse = cv2.bitwise_not(size_up_mask)
    
    roi = model_image[:256, interval-1:interval-1 + 192]
...
def final_process(model_image, composition_name, fg, bg, interval, isUpper, final):
    ...
    if isUpper == 0:
      ...
      model_image[:256, interval-1:interval-1 + 192] = coarse_image
