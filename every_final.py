...
from extract_mask_from_sel import otsu_external_contour
...
tf.flags.DEFINE_string("result_dir_stage2", "testdata/" + FLAGS.userId + "/output/composed_images/",
...
tf.flags.DEFINE_string("stage_dir", "stage/" + FLAGS.userId + "/stage",
                       "user's coarse image, mask_mat, mask image")

tf.flags.DEFINE_string("mall_name", "test_img", "shopping mall name")

tf.flags.DEFINE_string("prod_dir", "data/" + "test_img" + "/" + FLAGS.category_name + "/",
...
class model_stage2(object):
  ...
  def _process_image_stage2(self, prod_image, image_name, product_image_name, sess,
    ...
    mask_output = scipy.misc.imread(FLAGS.result_dir_stage1 + 
                                    image_name + "_" +
                                    product_image_name + "_gmmask.png")
    ...
    tps_image_before = tps_stn(img, nx, ny, p, [256,192,3])
    tps_mask_before = tf.cast(tf.less(tf.reduce_sum(tps_image, -1), 3*0.95), tf.float32)
    """
    
    tps_image = scipy.misc.imread(FLAGS.result_dir_stage1 + 
                                    image_name + "_" +
                                    product_image_name + "_gmm.png")
    """
    tps_mask = scipy.misc.imread(FLAGS.result_dir_stage1 + 
                                    image_name + "_" +
                                    product_image_name + "_gmmask.png")
    """
    tps_image = self.process_one_image(tps_image, resize_height, resize_width)
    tps_image = tf.reshape(tps_image, tf.stack([1, 256, 192, 3]))
    tps_mask = tf.cast(tf.less(tf.reduce_sum(tps_image, -1), 3*0.95), tf.float32)
    
    """
    tps_image = tf.convert_to_tensor(tps_image, dtype=tf.float32)
    tps_image = tf.expand_dims(tps_image, 0)
    tps_mask = tf.convert_to_tensor(tps_mask, dtype=tf.float32)
    tps_mask = tf.expand_dims(tps_mask, 0)
    """
    
    #tps_image = tf.reshape(tps_image, tf.stack([256, 192, 3]))
  ...
  def __init__(self, category_num, graph2):
    ...
    with self.graph.as_default() as graph:
      ...
      if category_num == '1001':
        category_name = "men_tshirts"
        FLAGS.checkpoint2 = "model/" + category_name + "/stage2/model-180000"
...
def save_product_mask(raw, mask, composition_name, tag=""):
  ...
  cv2.imwrite(FLAGS.result_dir_stage2 + composition_name + 'fg.png', cv2.cvtColor(fg, cv2.COLOR_RGB2BGR))
  with open(FLAGS.result_dir_stage2 + composition_name + '1_pkl.pkl', 'wb') as f4:
    pkl.dump(info_data, f4, pkl.HIGHEST_PROTOCOL)
  if tag=='save':
    return fg
def save_model_mask(model_image, raw, mask1, mask2, composition_name, interval, isUpper, tag=""):
  """
  2.26
  """
  ...
  if isUpper == 0:
    print('Is upper = 0')
    binary_segment_lower = cv2.imread(FLAGS.input_dir + "/body_segment/" + FLAGS.imageId + "_1.png", 0)
    resized_binary_segment_lower = cv2.resize(binary_segment_lower, (model_image.shape[1], model_image.shape[0]), interpolation=cv2.INTER_AREA)
    resized_binary_segment_lower = process_raw_mask(resized_binary_segment_lower)
    
    final_mask = cv2.add(resized_binary_segment_lower, mask1)
    final_mask = process_raw_image(final_mask)
    final_mask_reverse = cv2.bitwise_not(final_mask)
    
    bg = cv2.bitwise_and(model_image, model_image, mask=final_mask_reverse)
  elif isUpper == 1:
    binary_segment_upper = cv2.imread(FLAGS.input_dir + "/upper_segment/" + FLAGS.imageId + "_0.png", 0)
    resized_binary_segment_upper = cv2.resize(binary_segment_upper, (192, 256), interpolation=cv2.INTER_NEAREST)
    resized_binary_segment_upper = process_raw_mask(resized_binary_segment_upper)
    print("resized_binary_segment_upper_shape: " + str(resized_binary_segment_upper.shape))
    print("resized_binary_segment_upper_type: " + str(type(resized_binary_segment_upper)))
    """
    final_mask = cv2.add(resized_binary_segment_upper, mask1)
    print("1")
    final_mask = process_raw_mask(final_mask)
    """
    final_mask = cv2.add(resized_binary_segment_upper, mask1)
    final_mask = process_raw_mask(final_mask)
    final_mask = cv2.add(final_mask, mask2)
    final_mask = process_raw_mask(final_mask)
  ...
  cv2.imwrite(FLAGS.result_dir_stage2 + composition_name + 'bg.png', cv2.cvtColor(bg, cv2.COLOR_RGB2BGR))
...
def inference(image_names, product_image_names, image_name, product_image_name, model1, model2, isUpper, i, j):
  ...
  with tf.Session(config=config) as sess:
    (coarse_image, tps_image, mask_output, tps_mask) = model2._process_image_stage2(prod_image, image_name, 
                                        product_image_name, sess)
  
  scipy.misc.imsave(FLAGS.result_dir_stage2 + image_names[j] +
                          "_" + product_image_names[j] + '_tps_mask.png',
                          np.squeeze(tps_mask))
  ...
  print("test")
  print(type(model2.model_image_outputs))
  print(model2.model_image_outputs.shape)
  print(type(model2.select_mask))
  print(model2.select_mask.shape)
      ...
      while wait_queue.qsize() != 0:
        ...
        for i in range(FLAGS.begin, FLAGS.end, batch_size):
          ...
          for j in range(i, i + batch_size):
            ...
            FLAGS.stage_dir = "testdata/" + FLAGS.userId + "/stage"
            ...
            if not os.path.exists(FLAGS.result_dir_stage2 + middle_composition_name + "1_pkl.pkl"):
              ...
              #0318 tps_mask를 적용해본다.#
              composition_raw = _load_image(FLAGS.result_dir_stage2 + middle_composition_name + "final.png")
              composition_mask1 = cv2.imread(FLAGS.result_dir_stage1 + middle_composition_name + "mask.png", 0)
              composition_mask2 = cv2.imread(FLAGS.result_dir_stage2 + middle_composition_name + "sel_mask.png", 0)
              ...
              if(FLAGS.isUpper == 0):
                composition_raw = _process_ratio(model_image_height, model_image_width,
                                  composition_raw)
                composition_mask1 = _process_ratio(model_image_height, model_image_width,
                                  composition_mask1)
                composition_mask2 = _process_ratio(model_image_height, model_image_width,
                                  composition_mask2)
                
              
              
              composition_mask1 = process_raw_mask(composition_mask1) 
              composition_mask2 = otsu_external_contour(composition_mask2)
              
              final_mask = save_model_mask(model_image, composition_raw, composition_mask1, composition_mask2,
            ...
            """
            composition_raw = _load_image(FLAGS.result_dir_stage2 + middle_composition_name + "final.png")
            composition_mask = _load_image(FLAGS.result_dir_stage2 + middle_composition_name + "sel_mask.png")
            """
            ...
            coarse_final_dir = ""
            if FLAGS.isUpper == 0:
              coarse_final_dir = FLAGS.output + "/final_lower_images/"
            elif FLAGS.isUpper == 1:
              coarse_final_dir = FLAGS.output + "/final_upper_images/"
            
            scipy.misc.imsave(coarse_final_dir + middle_composition_name + "final.png", coarse_final_image)
            """
            #################################(#1 끝)#############################
            ...
            #두 이미지를 합쳐야 할 경우
              ...
              composition_mask1 = cv2.imread(FLAGS.result_dir_stage1 + composition_name + "mask.png", 0)
              composition_mask1 = process_raw_mask(composition_mask1)
              composition_mask2 = cv2.imread(FLAGS.result_dir_stage2 + composition_name + "sel_mask.png", 0)
              composition_mask2 = otsu_external_contour(composition_mask)

              # isUpper 값을 무조건 1로
              bg = save_model_mask(coarse_image, composition_raw, composition_mask1, composition_mask2, 
