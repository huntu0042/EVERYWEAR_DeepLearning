          ...
          for j in range(i, i + batch_size):
            ...
            with open(FLAGS.result_dir_stage2 + middle_composition_name + "1_pkl.pkl", "rb") as f1:
              try:
                ...
                print("pkl.load fail")
                break
            """ 2019.2.25 delete, and add
            final_process(model_image, middle_composition_name, prod_mask['product_mask'],
                          model_mask['model_mask'], interval, FLAGS.isUpper, 0)
            """
            coarse_mask = _load_image(FLAGS.stage_dir + middle_composition_name + "mask.png")
            if FLAGS.isUpper == 1:
              binary_segment_upper = _load_image(FLAGS.input_dir + "/upper_segment/" + FLAGS.imageId + ".jpg")
              ...
              final_process(model_image, middle_composition_name, fg, bg, interval, FLAGS.isUpper, 0)

            elif FLAGS.isUpper == 0:
              final_process(model_image, middle_composition_name, prod_mask, model_mask, interval, FLAGS.isUpper, 0)
