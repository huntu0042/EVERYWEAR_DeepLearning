...
tf.flags.DEFINE_string("warping_image_dir", "",
                       "Warping cloth directory.")
...
def _to_tf_example(image, decoder):
  ...
  with open(FLAGS.image_dir + image.image_id, "r") as f1:
    encoded_image = f1.read()
  with open(FLAGS.image_dir + image.product_image_id, "r") as f2:
    encoded_prod_image = f2.read()
  with open(FLAGS.warping_image_dir + image.product_image_id[:-5] + "1.png", "r") as f3:
    warping_prod_image = f3.read()
  
  """
  encoded_image = scipy.misc.imread(FLAGS.image_dir + image.image_id)
  encoded_prod_image = scipy.misc.imread(FLAGS.image_dir + image.product_image_id)
  warping_prod_image = scipy.misc.imread(FLAGS.image_dir + image.product_image_id)
  """
  """
  0321 : add warping image
  """
  """
  ...
  decoder.decode_png(warping_prod_image)
  """
  try:
  ...
  tf_example = tf.train.Example(features=tf.train.Features(feature={
        ...
        'warping_product_image': _bytes_feature(warping_prod_image)
        }))
...
def main(unused_argv):
  ...
  # Load pose keypoints.
  #pose_keypoints = pkl.load(StrToBytes(open(FLAGS.pose_dir, "r")))
  pose_keypoints = pkl.load(open(FLAGS.pose_dir, "rb"))
