...
%MASK_DIR= sprintf('%s%s%s', '../testdata/', userID, '/stage/tshirts_tps/00015000_'); % 모델의 학습수에 따라 바꿈
...
[image1, image2, DATA_ROOT_LIST, MASK_DIR_LIST] = textread('connection.txt', '%s %s %s %s');
%PATH = sprintf('%s%s%s', '../testdata/', userID, '/test2.txt');
%[image1, image2] = textread(PATH, '%s %s');
