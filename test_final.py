...
def inference(image_names, product_image_names, image_name, product_image_name, model1, model2, isUpper, i, j):
  ...
  with open("connection.txt", "w") as f3:
    ...
    f3.write(image_names[j] +" " + product_image_names[j] + " " + prod_image_dir + " " + FLAGS.result_dir_stage1)
    ...
    while wait_queue.qsize() != 0:
      ...
      for i in range(FLAGS.begin, FLAGS.end, batch_size):
        ...
        for j in range(i, i + batch_size):
          ...
          if (FLAGS.isUpper == 1) and (FLAGS.lowerId == "000000"):
            # 상의 합성만을 요청
            FLAGS.switch = 1
            product_image_name = FLAGS.upperId + "_1"
            composition_name = image_name + "_" + product_image_name + "_"

            model_image = _load_image(FLAGS.input_dir + "/body_resized/" + image_name + ".jpg")
            interval = int(image_data['resized_interval'])
            if not os.path.exists(FLAGS.result_dir_stage2 + composition_name + "1_pkl.pkl"):
              #기존 합치고
              ...
              save_model_mask(model_image, composition_raw, composition_mask, 
                            composition_name, interval, FLAGS.isUpper, tag="")
              save_product_mask(composition_raw, composition_mask, composition_name)
              #합성 수행
            
            post1 = time.time()
            with open(FLAGS.result_dir_stage2 + composition_name + "0_pkl.pkl", "rb") as f1:
              try:
                model_mask = pkl.load(f1, encoding = "latin-1")
              except EOFError:
                print("pkl.load fail")
                break
            with open(FLAGS.result_dir_stage2 + composition_name + "1_pkl.pkl", "rb") as f1:
              try:
                prod_mask = pkl.load(f1, encoding = "latin-1")
              except EOFError:
                print("pkl.load fail")
                break
            final_process(model_image, composition_name, prod_mask['product_mask'],
          ...
          elif (FLAGS.isUpper == 0) and (FLAGS.upperId == "000000"):
            # 하의 합성만을 요청
            FLAGS.switch = 2
            ...
            post1 = time.time()
            with open(FLAGS.result_dir_stage2 + composition_name + "0_pkl.pkl", "rb") as f2:
              try:
                model_mask = pkl.load(f2, encoding = "latin-1")
              except EOFError:
                print("pkl.load fail")
                break
            with open(FLAGS.result_dir_stage2 + composition_name + "1_pkl.pkl", "rb") as f2:
              try:
                prod_mask = pkl.load(f2, encoding = "latin-1")
              except EOFError:
                print("pkl.load fail")
                break
            final_process(model_image, composition_name, prod_mask['product_mask'],
          ...
          elif (FLAGS.isUpper == 1) and (FLAGS.lowerId != "000000"):
            # 하의를 입은 상태에서 상의 합성 요청
            # 바지를 먼저
            FLAGS.switch = 3
            product_image_name = FLAGS.upperId + "_1"
            product_image_name_2 = FLAGS.lowerId + "_1"
            
            composition_name = image_name + "_" + product_image_name + "_"
            composition_name_2 = image_name + "_" + product_image_name_2 + "_"
            composition_name_3 = image_name + "_" + product_image_name + "_" + product_image_name_2 + "_"
            
            model_image = _load_image(FLAGS.input_dir + "/body_resized/" + image_name + ".jpg")
            coarse_image = _load_image(FLAGS.output + "/final_lower_images/" + composition_name_2 + "final.png")
            interval = int(image_data['resized_interval'])
            if not os.path.exists(FLAGS.result_dir_stage2 + composition_name + "1_pkl.pkl"):
              #기존 합치고
              ...
              save_product_mask(composition_raw, composition_mask, composition_name)
              bg = save_model_mask(model_image, composition_raw, composition_mask,
                                   composition_name, interval, FLAGS.isUpper, tag='save')
              fg = save_product_mask(composition_raw, composition_mask, composition_name,
                                     tag='save')
              final_process(model_image, composition_name, fg, bg, interval, FLAGS.isUpper, 0)
            post1 = time.time()
            ...
            bg = save_model_mask(coarse_image, composition_raw, composition_mask,
                                 composition_name_3, interval,
                                 FLAGS.isUpper, tag='save')
            
            final_process(coarse_image, composition_name_3, fg, bg, interval, FLAGS.isUpper, 1)
            post2 = time.time()
            print("[post_processing] process time : " + str(post2 - post1))
            ...
            # 상의를 입은 상태에서 하의 합성 요청
            FLAGS.switch = 4
            product_image_name = FLAGS.lowerId + "_1"
            product_image_name_2 = FLAGS.upperId + "_1"
            composition_name = image_name + "_" + product_image_name + "_"
            composition_name_2 = image_name + "_" + product_image_name_2 + "_"
            composition_name_3 = image_name + "_" + product_image_name_2 + "_" + product_image_name + "_"

            model_image = _load_image(FLAGS.input_dir + "/body_resized/" + image_name + ".jpg")
            coarse_image = _load_image(FLAGS.output + "/final_upper_images/" + composition_name_2 + "final.png")
            interval = int(image_data['resized_interval'])
            model_image_height = coarse_image.shape[0]
            model_image_width = coarse_image.shape[1]
            if not os.path.exists(FLAGS.result_dir_stage2 + composition_name + "1_pkl.pkl"):
              #기존 합치고
            ...
            post1 = time.time()
            with open(FLAGS.result_dir_stage2 + composition_name + "1_pkl.pkl", "rb") as f4:
              try:
                prod_mask = pkl.load(f4, encoding = "latin-1")
              except EOFError:
                print("pkl.load fail")
                break
            ...
            composition_mask = _process_ratio(model_image_height, model_image_width,
                                      composition_mask)
            bg = save_model_mask(coarse_image, composition_raw, composition_mask,
                                 composition_name_3, interval, FLAGS.isUpper, tag='save')
